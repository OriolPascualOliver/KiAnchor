...
def launch_gui() -> None:
    ...
    class App(tk.Tk):
        def __init__(self) -> None:
            super().__init__()
            self.title("BoardBond STEP Modifier")
        ...
        def _build_ui(self) -> None:
            ...
            ttk.Button(file_frame, text="Browse...", command=self._browse_input).grid(row=0, column=2, padx=4)
            ttk.Button(file_frame, text="Load", command=self._load_step).grid(row=0, column=3, padx=4)
            ...
            ttk.Button(file_frame, text="Browse...", command=self._browse_output).grid(row=1, column=2, padx=4)
            ttk.Button(file_frame, text="Export", command=self._export_step).grid(row=1, column=3, padx=4)
            ...
            ttk.Button(toolbar, text="Disable selected", command=self._disable_selected).pack(side=tk.LEFT)
            ttk.Button(toolbar, text="Enable selected", command=self._enable_selected).pack(side=tk.LEFT, padx=4)
            ttk.Button(toolbar, text="Enable all", command=self._enable_all).pack(side=tk.LEFT, padx=4)
            ttk.Button(toolbar, text="Refresh analysis", command=self._reload_analysis_only).pack(side=tk.LEFT, padx=4)
        ...
        def _load_step(self) -> None:
            try:
                ...
                self.solids, self.pcb, self.components = analyze_step(input_path, cfg, self._log)
        ...
        def _reload_analysis_only(self) -> None:
            ...
            try:
                cfg = self._cfg_from_gui()
                input_path = Path(self.input_var.get().strip())
                self._log("Refreshing analysis...")
                self.solids, self.pcb, self.components = analyze_step(input_path, cfg, self._log)
                valid_indices = {c.solid.idx for c in self.components}
                self.disabled_indices = {i for i in prev_disabled if i in valid_indices}
                self.selected_component_idx = prev_selected if prev_selected in valid_indices else None
                self._fill_tree()
                self._redraw_preview()
            except Exception as exc:
        ...
        def _export_step(self) -> None:
            try:
                ...
                self._log(f"Disabled solids: {format_disable_list(self.disabled_indices) or '(none)'}")
                rc = run_process(input_path, output_path, cfg, set(self.disabled_indices), self._log)
                if rc == 0:
